...
type TxScope = "all" | "inflow" | "outflow";

const CATEGORY_COLORS: Record<string, string> = {
  payment: "bg-accent",
  merchant_pos: "bg-paper-pink",
  split_bill: "bg-paper-yellow",
  receive: "bg-stamp-green",
};
...
export default function AnalisaPage() {
  ...
  // Outflow: transactions where I'm the payer, within the selected period.
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-2xl border border-ink-line/40 bg-ink p-6">
              ...
              <p className="mt-2 font-display text-2xl font-semibold capitalize">
                {maxCategory ? t(categoryLabelKey(maxCategory[0])) : "—"}
              </p>
            </div>
          </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={cn("flex h-9 w-9 items-center justify-center rounded-lg", CATEGORY_COLORS[featCat] || "bg-text-muted", "bg-opacity-20")}>
                      ...
                      <span className={cn("font-mono text-sm font-medium", isOut ? "text-warn-amber" : "text-stamp-green")}>
                        {isOut ? "-" : "+"}
